...
@Dao interface GundemDao {
    ...
    @Insert(onConflict=OnConflictStrategy.IGNORE) suspend fun addSources(rows:List<SourceEntity>)
    ...
    @Query("SELECT * FROM events WHERE updatedAt > :after ORDER BY importance DESC LIMIT 100") suspend fun recentEvents(after:Long):List<EventEntity>
    ...
    @Query("SELECT * FROM events WHERE velocity > 5 AND noise < 50 ORDER BY velocity DESC,updatedAt DESC LIMIT 60") fun risingFeed():Flow<List<EventEntity>>
...
@Database(entities=[SourceEntity::class,RawItemEntity::class,EventEntity::class,EventItemEntity::class,EventVersionEntity::class,ScanHistoryEntity::class],version=1,exportSchema=false)
abstract class AppDatabase:RoomDatabase(){abstract fun dao():GundemDao
 companion object { @Volatile private var INSTANCE:AppDatabase?=null
  fun get(context:Context)=INSTANCE?: synchronized(this){ INSTANCE?:Room.databaseBuilder(context.applicationContext,AppDatabase::class.java,"gundem-radari.db").build().also{INSTANCE=it} }
 }
}
