...
@Dao interface GundemDao {
    ...
    @Query("SELECT * FROM events WHERE updatedAt > :after ORDER BY importance DESC LIMIT 140")
    ...
    @Query("SELECT * FROM events WHERE scope='turkey' AND noise < 50 ORDER BY importance DESC,updatedAt DESC LIMIT 60")
    fun turkeyFeed():Flow<List<EventEntity>>

    @Query("SELECT * FROM events WHERE scope='world' AND noise < 50 ORDER BY importance DESC,updatedAt DESC LIMIT 60")
    fun worldFeed():Flow<List<EventEntity>>

    @Query("SELECT * FROM events WHERE religionPriority > 0 AND noise < 70 ORDER BY religionPriority DESC,importance DESC,updatedAt DESC LIMIT 80")
    fun religionFeed():Flow<List<EventEntity>>
    ...
    @Insert
    suspend fun scan(row:ScanHistoryEntity)
