...
private val trivialEarthquake=listOf(
    ...
    "deprem nerede oldu","deprem oldu mu","hangi illerde hissedildi","son depremler","kandilli son depremler"
)
private val nationwideCritical=listOf(
    "genel seçim","cumhurbaşkanlığı seçimi","milletvekili genel seçimi","referandum","erken seçim",
    "seçim sonuçları","oy sayımı","ülke genelinde","türkiye genelinde","81 il","tüm türkiye",
    "olağanüstü hal","ohal","seferberlik","sokağa çıkma yasağı","anayasa değişikliği","anayasa değişikliği teklifi"
)
private val majorStateActions=listOf(
    "merkez bankası faiz","tcmb faiz","politika faizi","resmi gazete","resmî gazete",
    "kanun yürürlüğe girdi","anayasa mahkemesi","yüksek seçim kurulu","ysk",
    "cumhurbaşkanı kararı","bakanlar kurulu","meclis kabul etti","tbmm kabul etti"
)
private val massCasualtyWords=listOf(
    "toplu ölüm","çok sayıda ölü","çok sayıda kişi hayatını kaybetti","can kaybı",
    "hayatını kaybetti","yaşamını yitirdi","öldü","ölü","cenazesi","enkaz altında","kayıp"
)
private val severeDamageWords=listOf(
    "yıkıldı","yıkılan bina","ağır hasar","ciddi hasar","enkaz","çöktü","çökme","tahliye edildi","afet bölgesi"
)
private val majorConflict=listOf(
    "savaş ilan","savaş başladı","askeri harekat","askerî harekât","işgal","füze saldırısı",
    "hava saldırısı","toplu saldırı","sınır ötesi operasyon","terör saldırısı"
)
private val disasterWords=listOf(
    "sel felaketi","orman yangını","heyelan","çığ","tsunami","kasırga","hortum",
    "afet","baraj yıkıldı","baraj çöktü","yanardağ","volkan"
)

private fun normalized(title:String,summary:String):String =
    (" $title $summary ").lowercase(locale).replace('’','\'').replace(Regex("\\s+")," ")
...
private fun extractEarthquakeMagnitude(text:String):Double?{
    ...
    return patterns.asSequence().mapNotNull{it.find(text)?.groupValues?.getOrNull(1)}
        .mapNotNull{it.replace(',','.').toDoubleOrNull()}.firstOrNull{it in 1.0..10.0}
}

private fun extractDeathCount(text:String):Int?{
    val patterns=listOf(
        Regex("""\b(\d{1,4})\s+(?:kişi\s+)?(?:öldü|ölü|hayatını kaybetti|yaşamını yitirdi)\b"""),
        Regex("""\bölü sayısı\s+(\d{1,4})\b"""),Regex("""\bcan kaybı\s+(\d{1,4})\b""")
    )
    return patterns.asSequence().mapNotNull{it.find(text)?.groupValues?.getOrNull(1)?.toIntOrNull()}.maxOrNull()
}

fun scores(title:String,source:SourceEntity,sourceCount:Int=1,summary:String=""):Pair<Double,Double>{
    ...
    if(majorStateActions.any{t.contains(it)}){
        importance+=28.0
        if(t.contains("türkiye")||t.contains("tbmm")||t.contains("tcmb")||t.contains("cumhurbaşkanı"))importance+=10.0
    }
    ...
    }else if(t.contains("toplu ölüm")||t.contains("çok sayıda ölü")||t.contains("çok sayıda kişi hayatını kaybetti")){
        importance+=30.0
    }

    if(t.contains("savaş")||t.contains("çatışma"))importance+=18.0
    if(t.contains("seçim")&&!t.contains("yerel seçim"))importance+=18.0
    if(t.contains("türkiye")||t.contains("ülke genelinde")||t.contains("türkiye genelinde")||t.contains("81 il"))importance+=10.0
